...
func main() {
	...
	shoppingStore := sqlite.NewShoppingStore(db)
	shoppingService := shopping.NewService(shoppingStore)

	server := lofamhttp.NewServer(taskService, noteService, wishlistService, shoppingService, staticDir)
